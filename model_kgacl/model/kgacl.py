...
class KGACL(KnowledgeRecommender):


    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(KGACL, self).__init__(config, dataset)
    ...
    def forward_1(self):
        ...
        # Create Gaussian noise with customizable standard deviation
        x = self.std_dev + torch.zeros_like(ego_embeddings)
        noise = torch.normal(mean=torch.zeros_like(ego_embeddings), std=x).to(ego_embeddings.device)

        ego_embeddings_with_noise = ego_embeddings + noise
        embeddings_list = [ego_embeddings_with_noise]
        for aggregator in self.aggregator_layers:
            ego_embeddings_with_noise = aggregator(self.A_in_1, ego_embeddings_with_noise)
            norm_embeddings = F.normalize(ego_embeddings_with_noise, p=2, dim=1)
            embeddings_list.append(norm_embeddings)
    ...
    def forward_2(self):
        ...
        embeddings_list = [ego_embeddings_with_noise]
        for aggregator in self.aggregator_layers:
            ego_embeddings_with_noise = aggregator(self.A_in_2, ego_embeddings_with_noise )
            norm_embeddings = F.normalize(ego_embeddings_with_noise, p=2, dim=1)
            embeddings_list.append(norm_embeddings)
        kgat_all_embeddings = torch.cat(embeddings_list, dim=1)
        user_all_embeddings, entity_all_embeddings = torch.split(kgat_all_embeddings, [self.n_users, self.n_entities])
    ...
    def calculate_loss(self, interaction):
        ...
        user_rand_samples_tensor = torch.tensor(user_rand_samples).long()
        # 使用转换后的张量进行索引操作
        cts_embedding_1 = user_all_embeddings_1[user_rand_samples_tensor]
        cts_embedding_2 = user_all_embeddings_2[user_rand_samples_tensor]

        entity_rand_samples_tensor = torch.tensor(entity_rand_samples)

        # 将PyTorch张量转换为长整型张量，并确保在与 entity_all_embeddings_1 和 entity_all_embeddings_2 相同的设备上
        entity_rand_samples_tensor = entity_rand_samples_tensor.long().to(entity_all_embeddings_1.device)

        # 使用转换后的张量进行索引操作
        e_cts_embedding_1 = entity_all_embeddings_1[entity_rand_samples_tensor]
        e_cts_embedding_2 = entity_all_embeddings_2[entity_rand_samples_tensor]
    ...
    def update_attentive_A(self):
        ...
        drop_edge_1_tensor = torch.tensor(drop_edge_1).long()
        indices_1 = indices.view(-1, 2)[drop_edge_1_tensor].view(2, -1)
        drop_edge_1_tensor = torch.tensor(drop_edge_1).long()
        kg_score_1 = kg_score[drop_edge_1_tensor]
        ...
        drop_edge_2_tensor = torch.tensor(drop_edge_2).long()
        # 对 indices 进行索引操作
        indices_2 = indices.view(-1, 2)[drop_edge_2_tensor].view(2, -1)
        # 对 kg_score 进行索引操作
        kg_score_2 = kg_score[drop_edge_2_tensor]
